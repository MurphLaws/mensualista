...
const footerLinks = {
  ...
  empresa: [
    { label: "Registrar empresa", href: "/auth" },
    { label: "Iniciar sesion", href: "/auth" },
  ...
  return (
    <footer style={{ background: "#0F0A1A" }} className="py-16 px-4 sm:px-6 lg:px-8">
      <div className="container mx-auto max-w-6xl">
          ...
          {Object.entries(footerLinks).map(([key, links]) => (
            <div key={key}>
              ...
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="text-sm text-[#524D63] hover:text-white transition-colors">
                      {link.label}
                    </a>
                  </li>
                ))}
        ...
        >
          <p className="text-sm text-[#524D63]">&copy; {currentYear} Mensualista. Todos los derechos reservados.</p>
